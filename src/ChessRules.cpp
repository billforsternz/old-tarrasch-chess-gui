...
{    
    ...
    assert( sizeof(ChessPositionRaw) ==
                 (offsetof(ChessPositionRaw,full_move_count) + sizeof(full_move_count) + sizeof(unsigned long))
          );

    // We also rely on Moves being 32 bits for the implementation of Move
    //  bitwise == and != operators
    assert( sizeof(Move) == sizeof(unsigned long) );
...
Move ChessRules::Transform( Move move )
{
    ...
    switch( move.special )
    {           
        case SPECIAL_WK_CASTLING:       ret.special = SPECIAL_BK_CASTLING;
        case SPECIAL_BK_CASTLING:       ret.special = SPECIAL_WK_CASTLING;
        case SPECIAL_WQ_CASTLING:       ret.special = SPECIAL_BQ_CASTLING;
        case SPECIAL_BQ_CASTLING:       ret.special = SPECIAL_WQ_CASTLING;
        case SPECIAL_WPAWN_2SQUARES:    ret.special = SPECIAL_BPAWN_2SQUARES;
        case SPECIAL_BPAWN_2SQUARES:    ret.special = SPECIAL_WPAWN_2SQUARES;
        case SPECIAL_WEN_PASSANT:       ret.special = SPECIAL_BEN_PASSANT;
        case SPECIAL_BEN_PASSANT:       ret.special = SPECIAL_WEN_PASSANT;
    }
