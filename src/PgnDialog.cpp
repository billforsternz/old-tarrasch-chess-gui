...
//                                              break;
        ...
        case 10:s1=ptr_gds[item1].moves_txt;
                s2=ptr_gds[item2].moves_txt;    break;
        case 11:iflag = true;
    ...
    {                                 
        ...
        else
        {
            if( gc->gds[i].current_game )
                modified = modified || objs.gl->undo.IsModified();
            if( modified )
                strcpy( buf,"*" );
            if( game_nbr )
            {
                _itoa( game_nbr, buf, 10 );
                if( modified )
                    strcat( buf, " *" );
            }
        }
        ...
        std::string s = gc->gds[i].moves_txt;
        int len = s.length();
        if( len>=1 && s[len-1] == '*' )
            s = s.substr(0,len-1);
...
bool PgnDialog::LoadGame( GameLogic *gl, GameDocument& gd, int &file_game_idx )
{
    if( selected_game )
    {
        ...
        gd = *selected_game;
        gd.selected = false;
        selected_game->current_game = true;
...
void PgnDialog::OnBoard2Game( wxCommandEvent& WXUNUSED(event) )
{
    ...
    if( list_ctrl && list_ctrl->GetItemCount()==sz )
    {
        ...
        std::vector<GameDocument>::iterator iter = gc->gds.begin() + idx_focus;
...
void PgnDialog::OnCut( wxCommandEvent& WXUNUSED(event) )
{
    ...
    if( list_ctrl && list_ctrl->GetItemCount()==sz )
    {
        std::vector<GameDocument>::iterator iter = gc->gds.begin();
        std::vector<GameDocument>::iterator iter_focus;
