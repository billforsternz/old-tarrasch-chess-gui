...
bool GameLogic::EditingLog()
{
    ...
    bool editing_log = (log_filename == gc.pgn_filename);
    return editing_log;
}
...
{
    Atomic begin;
    string filename = objs.repository->log.m_file;
    if( filename == "" )
    {
...
void GameLogic::NextGamePreviousGame( int idx )
{
    ...
    bool editing_log = objs.gl->EditingLog();
    bool have_game = false;
    GameDocument *gd_file = &gc.gds[idx];
    have_game = gd_file->in_memory;
    if( !have_game )
    {
        FILE *pgn_in = pf.ReopenRead( gd_file->pgn_handle );
...
GameDocument *GameLogic::PutBackDocument()
{
    GameDocument *p=NULL;
    for( int i=0; !p && i<gc.gds.size(); i++ )
    {
    ...
    {
        if( gc_clipboard.gds[i].current_game  )
...
void GameLogic::IndicateNoCurrentDocument()
{
    ...
    {
        gc.gds[i].current_game = false;
        gc.gds[i].selected = false;
    }
    for( int i=0; i<gc_clipboard.gds.size(); i++ )
    {
        gc_clipboard.gds[i].current_game = false;
...
void GameLogic::StatusUpdate( int idx )
{
    if( objs.frame )
    {
        ...
        bool doc_modified = (gd.game_details_edited || gd.modified || undo.IsModified());
        str = doc_modified ? "*" : "";
        int nbr_modified=0;
        if( file_loaded )
        {
            for( int i=0; i<gc.gds.size(); i++ )
            {
                if( gc.gds[i].modified || gc.gds[i].game_details_edited )
                    nbr_modified++;
                else if( gc.gds[i].current_game )
                {
                    if( doc_modified )
                        nbr_modified++;
                }
            ...
            char buf[80];
            if( idx >= 0 )
                sprintf( buf, doc_modified?"* Game %d of %d":"Game %d of %d", idx+1, gc.gds.size() );
            else if( nbr_modified )
            {
                sprintf( buf, doc_modified?"* File games: %d (%d modified)":"File games: %d (%d modified)",
                     gc.gds.size(), nbr_modified );
            }
            else
            {
                sprintf( buf, doc_modified?"* File games: %d":"File games: %d",
                     gc.gds.size() );
            }
            str = buf;
        }
