...
BoardSetup::BoardSetup( wxBitmap *bitmap, wxWindow *parent )
{
    this->bitmap  = bitmap;
    this->parent  = parent;
	BITMAP info;
	...
	buf_board = new byte[mem_size = width_bytes*height];
    ...
    /*int ret=*/::GetBitmapBits((HBITMAP)(bitmap->GetHBITMAP()), width_bytes*height, buf_board );
    //DebugPrintf(( "In BoardSetup ::GetBitmapBits() returns %d\n",ret));
...
void BoardSetup::Put( char src_file, char src_rank, char dst_file, char dst_rank )
{
	...
	else
	{
		...
		{
			src			  =
				(unsigned long *)(buf_box   + Offset(src_file,src_rank) + i*width_bytes);
			dst			  =
				(unsigned long *)(buf_board + Offset(dst_file,dst_rank) + i*width_bytes);
